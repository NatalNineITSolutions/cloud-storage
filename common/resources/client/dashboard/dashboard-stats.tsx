...
export default function DashboardStats() {
  ...
  function formatFileSize(bytes:any) {
    ...
    const k = 1024;
    const dm = 2; // Decimal places to round to
  ...
  function fetchUserReport(payload: Payload): Promise<any> {
    return apiClient
      .get('user/reports', { params: payload })
      .then((response) => response.data);
  }

  // Usage example of calling the API
  function callUserReport() {
    const payload: Payload = { types: ['header'] };

    fetchUserReport(payload)
      .then((data) => {
        console.log('Report data:', data);
        setReportData({
          newFiles: data?.headerReport.newFiles || 0,
          newFolders: data?.headerReport.newFolders || 0,
          totalFileSize: data?.headerReport.totalFileSize || "0",
        });
      })
      .catch((error) => {
  ...
  const formattedFileSize = formatFileSize(parseInt(reportData.totalFileSize));
      ...
      <div className="grid grid-cols-1 md:grid-cols-4">
        <div className="dashboard-storage flex items-center px-40 gap-30 bg-white">
          ...
          <div>
            ...
            <div className="text-2xl font-bold">{reportData.newFiles}+</div>
          </div>
        ...
        <div className="dashboard-storage flex items-center px-70 gap-50 bg-white">
          <div>
            <div className="text-sm text-muted-foreground">New Folders</div>
            <div className="text-2xl font-bold">{reportData.newFolders}+</div>
          </div>
          <div>
        ...
        <div className="dashboard-storage flex items-center px-70 gap-50 bg-white">
          ...
          <div>
            ...
            <div className="text-2xl font-bold">{formattedFileSize}</div>
          </div>
        </div>
